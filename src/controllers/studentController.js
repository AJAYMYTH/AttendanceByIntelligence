...
exports.getStudents = async (req, res) => {
    ...
    try {
        console.log('[StudentController] Fetching students for section:', section);

        let query = supabase.from('students').select('id, name, register_number, section');
        if (section) {
            const decodedSection = decodeURIComponent(section);
            console.log('[StudentController] Using decoded section:', decodedSection);
            query = query.eq('section', decodedSection);
        }

        const { data, error } = await query;
        if (error) {
            console.error('[StudentController] Supabase error:', error);
            throw error;
        }

        console.log('[StudentController] Found', data?.length || 0, 'students');
        res.json(data || []);
    } catch (err) {
